    ...
    <div class="flex items-start space-x-4">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="mt-3 flex items-center space-x-4">
                <!-- Voting Buttons -->
                <div class="flex items-center space-x-2" id="vote-buttons-{{ $comment->id }}">
                    @auth
                        <button onclick="voteComment({{ $comment->id }}, 'like')"
                                class="flex items-center space-x-1 text-sm {{ $comment->isLikedBy() ? 'text-green-600' : 'text-gray-500' }} hover:text-green-600 transition-colors">
                            <svg class="h-5 w-5" fill="{{ $comment->isLikedBy() ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"></path>
                            </svg>
                            <span id="likes-count-{{ $comment->id }}">{{ $comment->likes_count }}</span>
                        </button>

                        <button onclick="voteComment({{ $comment->id }}, 'dislike')"
                                class="flex items-center space-x-1 text-sm {{ $comment->isDislikedBy() ? 'text-red-600' : 'text-gray-500' }} hover:text-red-600 transition-colors">
                            <svg class="h-5 w-5" fill="{{ $comment->isDislikedBy() ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.736 3h4.018a2 2 0 01.485.06l3.76.94m-7 10v5a2 2 0 002 2h.096c.5 0 .905-.405.905-.904 0-.715.211-1.413.608-2.008L17 13V4m-7 10h2m5-10h2a2 2 0 012 2v6a2 2 0 01-2 2h-2.5"></path>
                            </svg>
                            <span id="dislikes-count-{{ $comment->id }}">{{ $comment->dislikes_count }}</span>
                        </button>
                    @else

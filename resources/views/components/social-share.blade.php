...
<div class="flex items-center space-x-2">
    ...
    <a href="https://twitter.com/intent/tweet?url={{ urlencode($url) }}&text={{ urlencode($title) }}"
       target="_blank"
       rel="noopener noreferrer"
       class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-black text-white hover:bg-gray-800 transition-colors"
    ...
    <!-- Copy Link -->
    <button onclick="copyPostLink('{{ $url }}')"
            id="copy-link-btn"
            class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-300 text-gray-700 hover:bg-gray-400 transition-colors"
            title="Copy link">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
